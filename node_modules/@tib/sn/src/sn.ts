...
function postprocess(serial: string, options: SnOptions = {}) {
  if (options.hash) {
    const hash: DigestFn =
      typeof options.hash === 'function' ? options.hash : digest;
    serial = hash(serial);
  }
  if (options.size) {
...
function findCommands(options: SnOptions) {
  ...
  const cmd =
    typeof platform.cmd === 'function' ? platform.cmd() : platform.cmd;
  let keys =
    typeof platform.keys === 'function' ? platform.keys() : platform.keys;
